...
internal sealed class CrsConverter : JsonConverter<Crs>
{
        ...
        if (reader.TokenType == JsonTokenType.StartObject)
        {
            while (reader.Read())
            {
                ...
                if (reader.TokenType == JsonTokenType.PropertyName)
                {
                    ...
                    {
                        reader.Read();
                        value.Properties = new(JsonSerializer.Deserialize<Dictionary<string, string>>(ref reader, options));
                    }
                }
            }
        }
    ...
    public override void Write(Utf8JsonWriter writer, Crs value, JsonSerializerOptions options)
    {
        ...
        JsonSerializer.Serialize(writer, value.Properties.ToDictionary(x => x.Key, x => x.Value), options);
